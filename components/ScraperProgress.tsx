...
export default function ScraperProgress({ job }: ScraperProgressProps) {
  ...
  const percent = job.progress.total_pages > 0
    ? Math.round((job.progress.current_page / job.progress.total_pages) * 100)
    : 0;
    ...
    <div className="p-4 border border-gray-200 rounded-lg bg-white">
      ...
      {job.status === "running" && (
        <div className="mb-3">
          ...
          <div className="flex justify-between mt-1 text-xs text-gray-500" style={{ fontFamily: "var(--font-ibm-plex)" }}>
            <span>Page {job.progress.current_page}/{job.progress.total_pages}</span>
            <span>{job.progress.products_found} found</span>
          </div>
        </div>
      )}
      ...
      {job.status === "completed" && (
        <div className="flex gap-4 text-xs text-gray-600" style={{ fontFamily: "var(--font-ibm-plex)" }}>
          <span>{job.products_created} created</span>
          <span>{job.products_updated} updated</span>
        </div>
      )}
