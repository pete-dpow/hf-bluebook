...
CREATE TABLE IF NOT EXISTS cde_mail (
  ...
  from_user_id  UUID,
  to_user_id    UUID,
  status        TEXT NOT NULL DEFAULT 'OPEN' CHECK (status IN ('OPEN','OVERDUE','RESPONDED','CLOSED')),
...
CREATE TABLE IF NOT EXISTS cde_issues (
  ...
  assigned_to     UUID,
  priority        TEXT NOT NULL DEFAULT 'MEDIUM' CHECK (priority IN ('CRITICAL','HIGH','MEDIUM','LOW')),
  status          TEXT NOT NULL DEFAULT 'OPEN' CHECK (status IN ('OPEN','WORK_DONE','INSPECT','CLOSED')),
  raised_by       UUID,
